...
# Define ontology for entities, events, and relations
ontology = {
    'entities': ['vehicle', 'disease', 'ambulance', 'government', 'Khan'],
    'events': ['announce', 'dispatch', 'assist', 'arrest'],
    'relations': ['assist', 'respond', 'improve', 'arrest']
}
...
# Function to extract events and roles
def extract_events_and_roles(doc):
    events_roles = []
    for token in doc:
        if token.lemma_ in ontology['events']:
            roles = [(child.text, child.dep_) for child in token.children]
            events_roles.append((token.lemma_, roles))
    return events_roles
...
if content:
    ...
    # Extract events and roles
    st.subheader("Extracted Events and Roles")
    events_roles = extract_events_and_roles(doc)
    if events_roles:
        for event in events_roles:
            st.write(f"Event: {event[0]}, Roles: {event[1]}")  # Display "Event: Action, Roles"
    else:
    ...
    st.components.v1.html(html2, height=500)
